            ...
            <div class="link-list-wrapper">
                <ul class="link-list">
                    @foreach ($item['dropdown'] as $subitem)
                        @if (is_array($subitem))
                            <li><a class="list-item" href="{{-- $subitem['href'] --}}"
                                    @if (isset($subitem['target'])) target="{{ $subitem['target'] }}" @endif><span>{{ $subitem['text'] }}</span></a>
                            </li>
                        @elseif ($subitem === '-')
                        ...
                        <div class="link-list-wrapper">
                            <ul class="link-list">
                                @foreach ($submenu as $subitem)
                                    @if (is_array($subitem))
                                        <li>
                                            <a class="list-item" href="{{-- $subitem['href'] --}}"
                                                @if (isset($subitem['target'])) target="{{ $subitem['target'] }}" @endif><span>{{ $subitem['text'] }}</span></a>
                                        </li>
                                    @elseif ($subitem === '-')
                                        <li><span class="divider"></span></li>
...
@else
    <li class="nav-item">
        <a class="nav-link {{-- $item['active'] ? 'active' : '' --}}" href="{{ $item['url'] }}"
            @if (isset($item['target'])) target="{{ $item['target'] }}" @endif><span>{{ $item['text'] }}</span>
        </a>
    </li>
@endif
